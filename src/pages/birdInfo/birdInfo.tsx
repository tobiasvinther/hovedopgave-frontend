...
export default function BirdInfo() {
  ...
  const fetchBird = async () => {
    try {
      const response = await fetch(
        "http://localhost:8080/api/birds/" + selectedItem
      );
  ...
  return (
    <>
      <div
        style={{ marginLeft: "190px", marginRight: "190px", marginTop: "40px" }}
      >
        <Grid container spacing={2}>
          ...
          <Grid item xs={12}>
            <InfoList
              order={currentBird?.order}
              redList={currentBird?.redList}
            />
          </Grid>
          <Grid item xs={12}>
          ...
          <Grid item xs={12}>
            <Typography variant="h6">
              Seneste billeder af {selectedItem}
            </Typography>
          </Grid>
          <Grid item xs={12}>
